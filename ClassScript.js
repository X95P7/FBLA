		...
		for(var i=0; i < Tris.length; i++){
			...
			*/
			...
			ctx.beginPath();
				...
				ctx.fillStyle = "black";
				var adjust = ctx.measureText("Classes").width;
				ctx.fillText("Classes", (pTp(100,"w") / 2) - (adjust / 2),pTp(55,"h"));
...
ft.addEventListener("mouseenter",brChangeFL,false);
hk.addEventListener("mouseenter",brChangeHK,false);
cl.addEventListener("mouseenter",brChangeCL,false);
so.addEventListener("mouseenter",brChangeSO,false);

//BR
 BR = document.getElementById("ClassesMenu");
}
